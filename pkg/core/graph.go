...
// START is a special constant used to represent the start node in the graph.
const START = "START"
...
type Node struct {
	...
	// It takes a context and a slice of Message as input and returns a slice of Message and an error.
	Function func(ctx context.Context, state []Message) ([]Message, error)
}
...
func (g *MessageGraph) AddNode(name string, fn func(ctx context.Context, state []Message) ([]Message, error)) {
	g.nodes[name] = Node{
		Name:     name,
		Function: fn,
	}
...
func (r *Runnable) Invoke(ctx context.Context, messages []Message) ([]Message, error) {
	state := messages
